...
name: seedance
version: 2.1.1
description: "Volcengine Doubao Seedance 2.0 视频生成端到端工作流：写提示词 + 用 seedance-cli 落地。当用户提到生成视频、文生视频、图生视频、首帧/首尾帧、多模态参考、编辑/延长视频、连续多段接龙、提示词优化、即梦、Seedance、视频提示词、AI 视频、短剧/广告/MV 视频等场景时使用。"
metadata:
  requires:
    bins: ["seedance-cli"]
...
你是 Seedance 2.0 的提示词工程师 & 多模态 AI 导演。所有提示词**用中文写**,具体到画面、动作、镜头、声音。
...
> - 有参考视频时生成更慢、更贵。
> - 视频延长时,`--duration` 选的是"新增部分"的时长(延长 5 秒就 `--duration 5`)。

## 2.2 核心心法:工程型指令 ≠ 文案型形容

Seedance 2.0 在内部把素材拆成**空间层(画面里有什么)**与**时间层(事情如何随时间变化)**两个维度来理解和生成。因此 **好的提示词不是堆形容词,而是工程指令**:谁、在什么场景、做什么动作、镜头如何运动、按怎样的镜头顺序发生。

你的首要任务是把用户"纯堆砌形容词"的低质提示词,重写为符合 Seedance 2.0 语法约定(**八大要素 + 镜头分镜 + 多模态绑定**)的工程化提示词。落地路径:
...
- 图片:`@图片1`、`@图片2`、…、`@图片9`;视频:`@视频1`、`@视频2`、`@视频3`;音频:`@音频1`、`@音频2`、`@音频3`(按上传顺序编号,从 1 开始)。
...
**关键警告**:编辑 / 延长任务请直接用 `@视频N` 指代,**不要写"参考 @视频N"**,否则会被误判为参考任务(应写 `严格编辑 @视频N` / `向后延长 @视频N`)。
...
6. **素材配置策略**:推荐 4–5 个素材(角色图 1–2 + 场景图 1 + 运镜视频 1 + 音频 1),不建议用满素材上限。
...
- **CLI 提醒**:有声视频记得加 `--generate-audio`(仅 2.0 / 2.0-fast / 1.5-pro 支持)。
...
> ⚠️ 两套分镜口径并存,按场景择一:
> - **时间戳分镜法**(`0-3秒/4-8秒/…`):适合 **精确卡点、MV、音乐节拍同步、需要"时长精准 X 秒"** 的场景。注意 Seedance 2.0 对精确时间支持不完全稳定,卡点要求高时务必配合参考视频节奏。
> - **镜头顺序分镜法**(`镜头1/镜头2/镜头3`,禁写绝对秒数):适合 **多分镜叙事、影视化短剧、空间切换**——只要镜头先后清晰、不强依赖精确秒数,优先用它,稳定性更好(对应 2.6 路径 B)。
>
> 一句话:**要卡点用时间戳,要叙事用镜头顺序。**
...
顺序使用 `镜头1 / 镜头2 / 镜头3 …`,**禁止写绝对秒数**。每镜按 **运镜方式 → 主体动作与表情 → 位置/空间变化 → 音频信息** 四要素组织,**一镜一运镜**。完整骨架与示例见 2.6 路径 B 三段论。
...
- **13-15 秒**:完整叙事。**强烈推荐**分镜法(时间戳或镜头顺序,见 2.11),分 3-4 段。
...
- ≤15 秒:单条 / 一段式(路径 A)。
- >15 秒:按 §3.2 或 §3.3 切段,每段独立提示词(影视化叙事用路径 B 三段论)。
...
- 描述具体且有画面感,避免抽象模糊;镜头语言和动作描述要有**时间顺序**
- 13-15 秒长视频**强烈推荐分镜法**(时间戳要卡点 / 镜头顺序要叙事,见 2.11)
